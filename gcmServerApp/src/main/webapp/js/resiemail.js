	...
	$.ajax({
	               ...
	               // url: hostUrl+"/rest/sendChatInitiationReq", 
	                ...
	                success:function(data){
						...
						sessionId = chatUserSessionData['sessionId'];
						clearInterval(pageTimeout);
						launchChatUrlFinal = chatHostUrl+'/rest/chat/popChat?chatAlias='+chatAlias+'&userId='+userId+'&secureKey='+secureKey+'&scriptPos='+scriptPos+'&sessionId='+sessionId;
		...
		$("li[name=topBarChat]").each(function(){
			$(this).unbind('click');
			$(this).find("a").attr('href', '/support/livepage/chat');
...
function fetchFaqs(formType){
		...
		$.ajax({
			...
			data: {subject: vSubject, body: vBody, isBusiness: false},  
			dataType: 'JSON',
			success: function(dataV){ 
				...
				$.each(dummy, function(key){ 
						...
						//console.log(lolUrl);
						popUpArray.push("<li><span></span><a href=\""+lolUrl+"\" target='_blank' onclick='javascript:appendFaqs(\""+lolUrl+"\",\""+FormName+"\");'>"+dummyX[1]+"</a></li>");
						});
...
function showFaqsMsgEmail(popUpstring, formType){
	  ...
	  $.msg({
	    ...
	    //  klass: '/cvContactUs/css/live.css',
	      content: "<div class='finish-submitting-modal program modal--responsive'>"+
	      "<div panel class='padding-l first-popup-content'> <span id='closeFaqMsg' class='removepopup hidden-phone'><i ng-show='!model.img' ng-class='model.icon' class='ng-scope icon-remove' style='font-size: 20px;'></i></span>"+
	      "<header class='mobpanel__header hidden-desktop hidden-tablet'>"+
	        "<div class='container'>"+
	          "<h1>Finish submitting question</h1>"+
	          "<button id='finishSubmitNot2' class='btn btn--heading phone-close' ng-click='CommonHeaderCtrl.showSecondModel()'>Close</button>"+
	        "</div>"+
	      "</header>"+
	      "<div class='container'>"+
	      	"<h1> Finish submitting question </h1>"+
	        "<div class='alertwarning'>"+
	        "<div class='iconwrapper'><i class='ng-scope icon-exclamation-sign' ng-class='model.icon' ng-show='!model.img'></i><span></span></div>" +
	          "<span class='alerttext'>Your question hasn't been submitted yet.</span> </div>"+
	        "<div class='popup-content1'>"+
	          "<div class='popup-text'>"+
	           "<h4 class='text1'>The following answers might help you immediately.</h4>"+
	            "<p>(Answers open in a separate window.)</p>"+
	            "<ul>"+
	              	popUpstring+
	            "</ul>"+
	          "</div>"+
	      ...
	      "<footer class='panel__footer'>"+
	        "<div class='container'>"+
	          "<div class='row'>"+
	            "<div class='span5'>"+
	              "<button id='finishSubmitBtn' class='btn btn--primary span12'>Finish submitting question</button>"+
	            "</div>"+
	            "<div class='span7'>"+
	              "<input type='button' id='finishSubmitNot' class='btn btn--secondary del_margin btn-style bold toggle-width span3' value='Cancel'>"+
	              "</input>"+
	            "</div>"+
	          "</div>"+
	        "</div>"+
	  ...
	  "</div>",
	      afterBlock : function(){
	        ...
	        $( '#finishSubmitNot' ).bind( 'click', function(){
	        	self.unblock();
	        	reporting.trackClick(this,'Email Us | Cancel');
	        });
	        $( '#finishSubmitNot2' ).bind( 'click', function(){
	        	self.unblock();
	        	reporting.trackClick(this,'Email Us | Cancel');
		        });
